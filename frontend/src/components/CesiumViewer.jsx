...
import { getDamLabel } from './Sidebar.jsx'
...
const C_LOWER_WALL  = Cesium.Color.fromCssColorString('#f0a500').withAlpha(0.92)
...
const C_UPPER_WATER = Cesium.Color.fromCssColorString('#1565c0').withAlpha(0.40)
...
export default function CesiumViewer({ candidates, selected, heightM, showFlood, simResult, onSelect }) {
  ...
  // ── [핵심] 지형 밀착 벽체 — wallPts 기반 ─────────
  // wallPts = [[lon, lat, terrainElev], ...]
  // minimumHeights = 지형 고도 → 계곡 단면에 딱 붙는 아치형
  // maximumHeights = FSL (수평)
  const _drawTerrainWall = (v, wallPts, fsl, isUpper, ref) => {
    const wallColor = isUpper ? C_UPPER_WALL : C_LOWER_WALL
    const outlColor = isUpper ? C_UPPER_OUTL : C_LOWER_OUTL

    ref.current.push(v.entities.add({
      wall: {
        positions:      Cesium.Cartesian3.fromDegreesArray(wallPts.flatMap(p => [p[0], p[1]])),
        minimumHeights: wallPts.map(p => p[2]),      // 지형 고도
        maximumHeights: wallPts.map(() => fsl),       // FSL 수평면
        material:       wallColor,
        outline:        true,
  ...
  // ── 단순 수직 벽체 (wallPts 없을 때 fallback) ────
  const _drawSimpleWall = (v, ax, bed, fsl, ref) => {
  ...
  // ── 상부댐 저수면 ─────────────────────────────────
  // 축선 상류 방향으로 근사 저수면 폴리곤 생성
  const _drawUpperReservoir = (v, ax, fsl, ref) => {
    const [x1,y1] = ax.p1
    const [x2,y2] = ax.p2
    const dx = x2-x1, dy = y2-y1
    const len = Math.sqrt(dx*dx + dy*dy)
    const nx = (-dy/len) * 0.005   // 수직 방향 ~550m
    const ny = ( dx/len) * 0.005
    const ux = ( dx/len) * 0.010   // 상류 방향 ~1100m
    ...
    ref.current.push(v.entities.add({
      polygon: {
        ...
        height:      fsl,
        material:    C_UPPER_WATER,
        outline:     true,
        outlineColor:C_UPPER_OUTL.withAlpha(0.5),
        outlineWidth:1,
      },
    }))
  }
  ...
  const drawDam = useCallback(() => {
    ...
    // 하부댐: wallPts 있으면 지형 밀착, 없으면 단순 벽체
    if (selected.wallPts?.length >= 2) {
      _drawTerrainWall(v, selected.wallPts, fsl, false, damEntRef)
    } else if (axis) {
      _drawSimpleWall(v, axis, bed, fsl, damEntRef)
    }
    ...
    Object.entries(DAM_AXES)
      ...
      .forEach(([, ax]) => {
        ...
        _drawSimpleWall(v, ax, ax.bed, upperFsl, damEntRef)
        _drawUpperReservoir(v, ax, upperFsl, damEntRef)
